# src/spark/config.py
"""Configuration management for spark."""
...
class Config:
    ...
    skill_paths: list = field(default_factory=lambda: ["~/.spark/skills", "./.spark/skills"])
    security: SecurityConfig = field(default_factory=SecurityConfig)
    history: HistoryConfig = field(default_factory=HistoryConfig)


# Two-tier config paths
USER_CONFIG_PATH = Path.home() / ".spark" / "config.yaml"
PROJECT_CONFIG_PATH = Path.cwd() / ".spark" / "config.yaml"
...
def load_config(path: str | None = None) -> Config:
    ...
    Two-tier priority:
    1. User global: ~/.spark/config.yaml
    2. Project: ./.spark/config.yaml (auto-created if not exists)
    ...
    return Config(
        ...
        skill_paths=merged_config.get("skill_paths", ["~/.spark/skills"]),
